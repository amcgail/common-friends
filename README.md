...
| `04 degree distribution visual.py` | Per-network degree-distribution histograms with M_k annotations.                                                                                                                                                                     |
| `05 tradeoff figure.py`            | Lift–cost trade-off: P_k(d > d_{pQ}) vs \bar s_k or \bar s_k/N (App. E + F_{i,k} weights), k = 1,\ldots,15, Q \in 90,95,99. Writes `figures/tradeoff_p{Q}_frac.png` and `figures/tradeoff_p{Q}_sbar.png` plus `tables/tradeoff.tsv`. |
